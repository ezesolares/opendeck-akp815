...
pub fn opendeck_to_device(_kind: &Kind, key: u8) -> u8 {
    if key < 15 {
        [12, 9, 6, 3, 0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2][key as usize]
    } else {
        key
    }
}
...
pub fn device_to_opendeck(_kind: &Kind, key: usize) -> usize {
    if key < 15 {
        [4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11, 0, 5, 10][key]
    } else {
        key
    }
}
...
fn read_button_press(input: u8, state: u8) -> Result<DeviceInput, MirajazzError> {
    ...
    button_states.extend(vec![0u8; 15 + 1]);
